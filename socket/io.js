const userModel = require("../../chat-server/models/user")
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("request-user-chat", async (data) => {
            try {
                ...
                let sorted = await chatModel.find().or([{ to: data.userID, from: usersJoined[socket.id].id }, { from: data.userID, to: usersJoined[socket.id].id }]).and([{ createdAt: { $lte: todt } }]).limit(15).sort({ createdAt: -1 })
