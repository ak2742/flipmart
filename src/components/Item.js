...
export default function Item(props) {
    ...
    return (
        <div className="card my-2 mx-2" style={{ "width": "18rem" }}>
            <img src={img} className="card-img-top" id='img_acc' alt={title} />
            <div className="card-body">
                ...
                {!my ? <Link to={`/products/1/${usr}/${id}`} className="btn btn-primary">Open</Link> :
                    <Link to={`/products/0/${usr}/${id}`} className="btn btn-primary">Open</Link>}
            </div>
        </div>
    )
}
