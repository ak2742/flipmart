...
export default function Idetails(props) {
    ...
    useSelector(state => state.prod).then(arr => { updateItem(arr) })
    ...
    useSelector(state => state.usr).then(arr => { updateUsr(arr) })
    ...
    return (
        <div className="container">{usr.name === undefined || item.title === undefined ? <h2 className='text-center'>!!! Incorrect URL !!!</h2> :
            <div className="card my-5 mx-2">
                <img src={item.image ? item.image : ""} className="card-img-top" id='img_det' alt={item.title ? item.title : ""} />
