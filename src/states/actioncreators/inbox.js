...
const sortData = () => {
  ...
  users = users.sort((a, b) =>
    inbox.findIndex(e => e.from === a._id || e.to === a._id) < inbox.findIndex(e => e.from === b._id || e.to === b._id) ? 1 : -1)
}
...
export const listSentMsg = (msg) => {
  return async (dispatch) => {
    getData()
    inbox.push(msg)
    let i = users.findIndex(e => e._id === msg.from || e._id === msg.to)
    ...
    for (let msgi = 0; msgi < msgs.length; msgi++) {
      ...
      if (i === -1) {
        ...
        if (n === -1) {
          ...
          if (openUser !== users[n]?._id && (msg.status === undefined || msg.status === "sent")) {
            users[n].new = (users[n].new || 0) + 1
          }
          else {
            users[n].new = users[n].new || 0
          }
        }
      }
    }
