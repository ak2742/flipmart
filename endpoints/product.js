  ...
  async (req, res) => {
    try {
      ...
      if (!reqUser.isSeller === "true") {
        return res.status(400).send({error: "you cannot upload products as you are not a seller"})
      }
...
router.put("/edit/:id", middleware, async (req, res) => {
  try {
    ...
    const { image, title, description, category, price } = req.body
    if (!reqUser.isSeller === "true") {
      return res.status(400).send({error: "you cannot edit products as you are not a seller"})
    }
    let newItem = {};
    if (image) {
    ...
    const productUpdated = await prodModel.findById(req.params.id)
...
router.delete("/delete/:id", middleware, async (req, res) => {
  try {
    const reqUser = await userModel.findById(req.user.id)
    if (!reqUser.isSeller === "true") {
      return res.status(400).send({error: "you cannot delete products as you are not a seller"})
    }
    const existingProduct = await prodModel.findById(req.params.id);
    if (!existingProduct) {
      return res.status(404).send({error: "Product Not Found"});
