...
router.post("/join",
    ...
    async (req, res) => {
        try {
            ...
            const token = await otphandler.newotp(createdUser)
...
router.post("/resendmail",
    async (req, res) => {
        try {
            ...
            newToken = await otphandler.newotp(matched.user, matched.updatePwd)
            res.json({ token: newToken, message: "Sent successfully" })
        } catch (error) {
            console.error(error.message);
            res.status(500).send({ error: "INTERNAL SERVER ERROR" })
        }
...
router.post("/passwordlost",
    ...
    async (req, res) => {
        try {
            ...
            const token = await otphandler.newotp(existingUser, true)
