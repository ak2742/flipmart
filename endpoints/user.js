...
router.post("/join",
    ...
    async (req, res) => {
        try {
            ...
            res.status(400).send("you must choose atleast one role for you")
        } catch (error) {
            console.log(error);
...
router.get("/find/:id", async (req, res) => {
    try {
        ...
        const userData = await userModel.findById(userId).select("-password").select("-email").select("-balance");
        res.json(userData);
    } catch (error) {
        console.log(error);
        res.status(500).send("INTERNAL SERVER ERROR")
...
router.delete("/account/delete",
    ...
    middleware, async (req, res) => {
        try {
            ...
            res.json({ Success: "Account and related products has been deleted", Account: deletedUser, Products: deletedProducts });
        } catch (error) {
            console.log(error);
            res.status(500).send("INTERNAL SERVER ERROR")
        }
